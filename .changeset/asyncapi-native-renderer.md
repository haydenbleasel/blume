...
**Breaking change:** AsyncAPI references were previously always rendered by the embedded Scalar SPA, and a 1.x site inherits the new renderer — with new routes, new sidebar entries, and new search and `llms.txt` records — on upgrade. Pin the previous output with `asyncapi: { renderer: "scalar" }`.
