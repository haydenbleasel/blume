...
Jeder Eintrag benötigt ein `href` (interne Route oder externe URL) und ein `label`, dazu optional ein `icon` — ein integrierter Iconname, der neben dem Label angezeigt wird und standardmäßig ein Datei-Symbol ist. Lass `popular` weg oder leer, um den Fallback auf die Seitenleiste beizubehalten.

Anders als Icons an anderen Stellen in Blume muss `icon` hier ein integrierter Name sein: Diese Zeilen werden in einer Client-Island gerendert, sodass ein Bildpfad oder Inline-SVG nicht unterstützt wird und auf das Datei-Symbol zurückfällt.
