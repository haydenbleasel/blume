...
Dot-folders are skipped, including Obsidian's own `.obsidian` config directory — which the dev watcher also ignores, so moving a pane in the app doesn't rebuild your site. Editing a note does. The directories no content scan reads (`node_modules`, `dist`, `.git`, …) are skipped too, so a vault rooted at the project itself doesn't publish dependency READMEs.
