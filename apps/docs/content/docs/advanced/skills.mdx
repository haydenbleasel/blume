---
title: Skills
description: The agent skills Blume ships — blume for building a site with a coding agent, blume-migrate for converting an existing docs site, and blume-update-docs for keeping docs in sync with your product.
---
...
`blume-migrate` converts an existing docs site — Mintlify, Docusaurus, Fumadocs, Nextra, Starlight, or anything else — into an idiomatic Blume project: config translated to `blume.config.ts`, navigation moved into the file tree, callouts rewritten to directives, icons mapped to Lucide. It's an important enough job to have [its own page](/docs/advanced/migrate).
