...
The reference is the `graphql()` adapter from `blume/reference`, listed under `reference` beside any [OpenAPI or AsyncAPI](/docs/advanced/api-reference) adapters:
...
Unlike `openapi()` and `asyncapi()`, `graphql()` takes no `renderer`: the embedded [Scalar](/docs/advanced/api-reference#the-scalar-renderer) reference reads OpenAPI documents only, so a GraphQL reference is always rendered natively.
