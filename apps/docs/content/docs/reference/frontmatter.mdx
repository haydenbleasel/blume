...
`display` sets the render mode of the page's folder group ([per-group overrides](/docs/content/navigation#per-group-overrides)) and is only meaningful on a folder's `index` page — anywhere else it has no group to configure, and Blume warns with `BLUME_SIDEBAR_DISPLAY_IGNORED`.
