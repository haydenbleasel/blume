...
The generated skill is a guide to your docs, not a summary of your product. To write that yourself, add a skill with the same name to [`agents.skills`](#skills-discovery). It replaces the generated one in the index, and at `/skill.md` too when it's a lone `SKILL.md`. A `public/skill.md` you ship wins over both. To turn the skill off:
