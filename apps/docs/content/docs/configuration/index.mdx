...
A dark mark disappears against dark browser chrome, so you can ship a second file for dark mode. Add a `-dark` sibling of any icon name — `icon-dark.svg`, `favicon-dark.png`, and so on — and Blume emits both icons behind a `prefers-color-scheme` media query:
...
The dark file is optional; with only one icon, Blume emits a single tag as before. Blume's own fallback mark ships both variants.
