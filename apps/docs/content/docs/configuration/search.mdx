...
By default, Blume indexes each page's title, description, and body reduced to plain text — fenced code blocks, images, and markup are stripped, so results stay relevant. The index is built from your source files, so it's identical in dev and production.

If your docs rely on code examples for searchable terms like options, methods, or error names, you can include fenced code block contents in the source-file search indexes:
...
Inline code is always searchable. `includeFencedCodeBlocks` applies to Orama, FlexSearch, and hosted provider records; Pagefind indexes rendered HTML instead.
