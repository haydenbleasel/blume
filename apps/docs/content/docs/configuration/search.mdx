...
For Orama, FlexSearch, Algolia, Orama Cloud, and Typesense, Blume indexes each page's title, description, and body reduced to plain text — code blocks, images, and markup are stripped, so results stay relevant. These indexes are built from your source files, so they're identical in dev and production.

If your docs rely on code examples for searchable terms like options, methods, or error names, you can include code block contents in the source-file search indexes:
