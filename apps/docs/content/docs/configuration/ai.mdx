...
Blume also ships a [`blume-update-docs` skill](/docs/advanced/self-updating-docs) for the other direction: a scheduled agent audits your merged PRs against the docs, fixes factual drift, and opens a `blume/*` PR — pairing naturally with `llms.txt`, raw Markdown URLs, and the MCP server, since agents read the published docs in the same shape users and models see.
