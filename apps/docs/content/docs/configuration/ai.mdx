...
It's one of [three skills Blume ships](/docs/advanced/skills) — alongside `blume-migrate` for converting an existing docs site and `blume-update-docs` for keeping docs in sync with your product from a scheduled agent run.
