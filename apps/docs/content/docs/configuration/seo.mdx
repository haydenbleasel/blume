...
:::note
The accent accepts a named preset (`blue`, `teal`, …) or any CSS color (hex, `oklch(…)`, `rgb(…)`, …); the other palette colors are hex.
:::

Emoji in a page title or site title render as [Twemoji](https://github.com/jdecked/twemoji) glyphs, fetched from a CDN while the card renders — so a build whose titles contain emoji needs network access.
