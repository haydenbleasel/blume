...
上で説明した内容はすべてそのまま引き継がれます: `route`、`label`/`route` 付きの `sources`、`expandSchemas`、[ソースごとのインデックス](#ソースごとのインデックス)のフラグ、そして操作のサマリーとタグによる検索インデックスです。`renderer: "scalar"` を設定すると埋め込みの Scalar SPA に戻れます — Scalar はドキュメントタイプを自動検出し、チャネル、操作、メッセージ、Models セクションをレンダリングします（イベント向けのインタラクティブなプレイグラウンドはありません）— そこでは、OpenAPI と同様に `noindex` のみが適用されます。
