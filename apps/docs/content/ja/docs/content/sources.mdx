...
インテグレーショントークンは `NOTION_TOKEN` 環境変数から取得されます（データベースをインテグレーションと共有してください）。デフォルトではすべてのページがインポートされます。`Status` プロパティを公開ゲートとして扱うには `publishedValue` を設定してください。その場合、それ以外の値は `draft: true` にマッピングされ、本番ビルドでは除外されます。**Notion の画像 URL は署名付きで有効期限がある**ため、アダプターはビルド時にそれらをダウンロードしてサイトのアセットに取り込み、参照を書き換えます。CMS の画像が静的ビルドを壊すことはありません。
