...
APIs orientadas a eventos usam um bloco irmão `asyncapi` com o mesmo formato — e o mesmo renderizador nativo. Cada operação `send`/`receive` se torna uma página real com tabelas de esquema do payload e dos cabeçalhos da mensagem, parâmetros de canal, bindings de protocolo e uma seção de Autorização derivada dos `securitySchemes` da especificação (em nível de servidor e de operação, com alternativas como grupos "ou"). Apenas a rota padrão difere (`/events`):
...
Tudo o que foi documentado acima se mantém sem alterações: `route`, `sources` com `label`/`route`, `expandSchemas`, as flags de [indexação por fonte](#indexação-por-fonte) e a indexação de busca por resumo e tag da operação. Definir `renderer: "scalar"` volta a optar pelo SPA incorporado do Scalar — o Scalar detecta automaticamente o tipo de documento e renderiza canais, operações, mensagens e uma seção de Modelos (sem playground interativo para eventos) — onde, como no OpenAPI, apenas `noindex` se aplica.
