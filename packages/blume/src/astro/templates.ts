 ...
 * The whole reference UI is owned by Scalar: the page renders the embed as a
 * prerendered, self-contained document (static even in server output) that loads
 * Scalar's standalone bundle from a CDN. It deliberately skips Blume's layout —
 * it is its own route, not part of the docs shell.
 */
...
<ScalarComponent configuration={configuration} renderMode="static" />
`;
