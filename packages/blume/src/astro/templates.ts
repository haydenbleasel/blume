 ...
 * handler and names a listed origin on every response. All three travel in one
 * options object so a new call site can't silently drop one of them.
 */
export const askEndpointTemplate = (
  ...
  // answers `OPTIONS` and stamps a listed origin on every response — errors
  // included, so a cross-origin caller can tell a 400 from a 500. Unlisted
  // origins get no header and stay subject to the same-origin rule. Left out
  // entirely when nothing is listed, so the default route is unchanged.
  const cors = options?.cors ?? [];
  const corsSetup =
    cors.length > 0
      ? `
const ALLOWED_ORIGINS = new Set(${JSON.stringify(cors)});
...
`
      : "";
  const responseInit = (status: number): string =>
    cors.length > 0
      ? `{ headers: corsHeaders(request), status: ${status} }`
      : `{ status: ${status} }`;
  const streamInit = cors.length > 0 ? "{ headers: corsHeaders(request) }" : "";
...
${keyCheck}
  try {
${stream}
    return result.toTextStreamResponse(${streamInit});
  } catch {
    return new Response("Failed to generate a response.", ${responseInit(500)});
  }
};`;
  return `// Generated by Blume. Do not edit.
${imports.join("\n")}
