...
go under `blog/` and changelog entries under `changelog/`.

Changelog entries are also collected into a generated **`/changelog`** timeline —
each one rendered newest-first with its date, version, and `category` in a left
rail beside its content. The page appears automatically once you have at least
one `type: changelog` entry (and nothing already occupies the `/changelog` route).

## Feeds

Blume generates an RSS feed automatically for each content type listed in
[`rss.types`](/configuration/seo#rss-feeds) — `blog` and `changelog` by default —
