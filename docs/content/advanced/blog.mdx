...
[changelog](/advanced/changelog), there's no generated index page — you compose
...
Feeds need an absolute site URL, so set [`deployment.site`](/deployment); Blume
...
When [structured data](/configuration/seo#structured-data) is on, each post is
...
To list posts automatically instead, add a [custom page](/advanced/custom-pages)
...
See [Custom Pages](/advanced/custom-pages#using-the-site-layout) for wrapping this
...
<CardGroup cols={2}>
  ...
  <Card title="SEO" href="/configuration/seo" icon="file">
