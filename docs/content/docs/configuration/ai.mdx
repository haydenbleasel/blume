...
Use Blume's [self-updating docs skill](/docs/advanced/automated-docs) with a
user-configured Codex, Cursor, Claude, or cron automation to audit your docs for
stale user-facing content and open a `blume/*` PR when an update is needed. The
workflow pairs well with `llms.txt`, raw Markdown URLs, and the MCP server
because agents can read the published docs in the same shape users and models
see.
