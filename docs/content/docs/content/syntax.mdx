...
Mark a TypeScript block `twoslash` to run the compiler over it: hover any token
to see its inferred type, and add an inline `^?` query to pin a type below the
line. Turn it on with `markdown: { code: { twoslash: true } }`.
