...
docs/guides/_meta.$.json   (folder meta applied to every locale)
```

A locale-specific `_meta.json` still overrides the shared one for that language.
...
:::note
Right-to-left locales set `<html dir="rtl">`, but a full RTL pass on the default
theme is still in progress — some layout details won't mirror yet.
:::
