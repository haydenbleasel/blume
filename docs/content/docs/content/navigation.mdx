...
Add a `meta.ts` to a folder to control how its group appears and how its pages
are ordered. Export a `defineMeta` object for full type-checking:
...
(sync or `async`) instead of an object to build it at scan time:
