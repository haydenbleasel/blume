...
Blume is an open-source, markdown-first documentation framework. Drop Markdown or
MDX into a folder, run `blume dev`, and get a production-grade docs site —
navigation, search, theming, Open Graph images, and a rich component library —
with no app boilerplate to write or maintain.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration" href="/configuration" icon="file">
...
Every Blume site can emit [`llms.txt` and `llms-full.txt`](/configuration/ai) so coding
...
files, [search](/configuration/search) works in dev and production without a hosted
...
- **Components** — callouts, cards, steps, tabs, accordions, badges, file trees,
  and API-reference blocks, usable in MDX with [no imports](/content/components).
...
- **API docs** — `blume import openapi` turns a spec into editable reference pages.
- **SEO** — metadata, Open Graph images, RSS feeds, and JSON-LD, [built in](/configuration/seo).
...
<CardGroup cols={2}>
  ...
  <Card title="AI" href="/configuration/ai" icon="lightbulb">
