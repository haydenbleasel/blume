...
At build time Blume writes a `sitemap.xml` of every indexable page and a
`robots.txt` that points crawlers to it. Both are on by default:
...
The sitemap needs an absolute [`deployment.site`](/deployment) and lists every
page except drafts, hidden, and `noindex` pages. The `robots.txt` allows all
crawlers and adds a `Sitemap:` line when the sitemap is available:
...
Ship your own `public/robots.txt` or `public/sitemap.xml` to take over — Blume
never overwrites a file you place in `public/`.
